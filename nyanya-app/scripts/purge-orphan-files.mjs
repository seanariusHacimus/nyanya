 ...
 *
 * Нужен в двух случаях: база уже очищена, а файлы удалить было нечем (без
 * AWS_* delete-accounts.mjs печатает оставшиеся ключи строкой «AWS_* не заданы
 * — … файлов осталось в хранилище: …» — их и складывают в список), либо файл
 * записан в хранилище, а транзакция отката не дала строке появиться в базе.
