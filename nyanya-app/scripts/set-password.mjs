 ...
 *
 * Обычный путь для человека — «Забыли пароль?» на странице входа: код на почту
 * и новый пароль. Скрипт нужен для случаев, когда доступа к почте аккаунта нет
 * (адрес заведён администратором, ящик потерян). Прочитать забытый пароль он не
 * умеет и не может: в базе хранится только отпечаток (scrypt с индивидуальной
