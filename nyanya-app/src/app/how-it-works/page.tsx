...
export const metadata = {
  ...
  description:
    "От выбора категории до первого рабочего дня — весь путь прозрачен. Как устроен подбор специалистов на nyanya.uz.",
};
...
const steps = [
  {
    ...
    text: "Няня, сиделка, помощник по хозяйству или водитель — каждая категория ведёт в каталог с нужным фильтром.",
  },
  {
    icon: MagnifyingGlass,
    title: "Изучите анкеты",
    text: "Фильтры по району, цене и опыту. Анкеты, опыт работы и отзывы помогают сравнить и выбрать.",
  },
  {
    icon: LockKeyOpen,
...
// §6.4 — что может содержать анкета. Документы — по желанию специалиста
// (решение владельца, 2026-09-11); перечень тот же, что на странице размещения.
const checks = [
  {
    icon: IdentificationCard,
    title: "Паспорт или ID-карта",
    text: "Специалист может загрузить паспорт по желанию.",
  },
  {
    icon: FirstAid,
    title: "Медицинские справки",
    text: "ВИЧ/СПИД, ЗППП, туберкулёз и общее состояние здоровья.",
  },
  {
    icon: GraduationCap,
    title: "Психоневрологический и наркологический диспансеры",
    text: "Справка о том, что специалист не состоит на учёте.",
  },
  {
    icon: EnvelopeSimple,
    title: "Отсутствие судимости",
    text: "Справка из органов внутренних дел; водителям — ещё и удостоверение.",
  },
];

// §6.6 — что происходит после открытия контактов (сервис бесплатный)
const afterUnlock = [
  {
    icon: UserPlus,
    text: "Открытие контактов бесплатно — нужен только аккаунт.",
  },
  {
    icon: InfinityIcon,
...
const faq = [
  {
    ...
    a: "Нисколько. Сервис бесплатный: нужен только аккаунт.",
  },
  {
    q: "Есть ли платные услуги?",
    a: "Нет. Всё бесплатно: каталог, просмотр анкет и открытие контактов. Подписок и скрытых списаний нет.",
  },
  {
    q: "Как размещаются анкеты специалистов?",
    a: "Специалист самостоятельно заполняет анкету, указывает информацию о себе и добавляет фотографию. По желанию можно приложить рекомендации, документы и другие сведения. После модерации анкета становится доступна на платформе для работодателей.",
  },
  {
    q: "Что делать, если специалист не подошёл?",
    a: "Вернитесь в каталог и выберите другого — фильтры и отзывы помогут сузить поиск. Решение о найме всегда остаётся за вами.",
  },
  {
    q: "Кто принимает решение о выборе специалиста?",
    a: "nyanya.uz помогает работодателям и специалистам находить друг друга. Работодатель самостоятельно знакомится с анкетой специалиста, проводит собеседование и принимает окончательное решение о сотрудничестве.",
  },
  {
    q: "Можно ли оставить отзыв?",
    a: "Да, если вы открывали контакты специалиста: оценку и отзыв можно оставить на странице его анкеты. Отзыв появляется в анкете и учитывается в общей оценке после проверки модератором.",
  },
];
...
export default function HowItWorksPage() {
  return (
    <main className="flex-1">
      ...
      {/* §6.4 — что может содержать анкета */}
        ...
        <Reveal>
          <div className="rounded-[2px] bg-cream-deep px-8 py-14 sm:px-12 lg:px-14">
            <h2 className="max-w-md font-display text-3xl leading-[1.12] font-medium text-ink sm:text-4xl">
              Что может содержать анкета специалиста
            </h2>
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-ink-soft">
              Кроме фотографии и рассказа о себе специалист может по желанию
              приложить документы — они повышают доверие семей и открывают
              «Премиум-профиль».
            </p>
        ...
        <Reveal>
          <div className="rounded-[2px] bg-charcoal px-8 py-14 text-cream sm:px-12 lg:px-14">
            ...
            <p className="mt-10 border-t border-cream/10 pt-6 text-sm leading-relaxed text-cream/60">
              nyanya.uz — платформа для поиска специалистов. Условия работы и
              оплату труда вы согласуете со специалистом самостоятельно.
            </p>
          </div>
        </Reveal>
      </section>
