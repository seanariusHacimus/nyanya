 ...
 *
 * Лимит Better Auth по IP (3 запроса за 10 секунд, счётчики в таблице
 * `rate_limit`) не защищает конкретный аккаунт от перебора с многих адресов.
 * Поэтому неудачи считаются ещё и по адресу почты:
 *
 ...
 *   IP вместе закрывают вход в аккаунт отовсюду на 60 минут. Это ограничение
 *   для ботнета: с одного IP до него не дойти — мягкий уровень пропускает с
 *   одного IP не больше 11 последовательных попыток за окно в 60 минут.
 *
 * Порог не строгий при одновременных запросах. Блокировка проверяется ДО
 * проверки пароля (hooks.before), а неудача засчитывается ПОСЛЕ неё
 * (hooks.after), поэтому запросы, уже прошедшие проверку блокировки, когда
 * засчитывается пороговая неудача, всё равно проверят пароль и будут учтены.
 * Синхронная пачка с N адресов получает сверх порога до ~3·N попыток на
 * каждый блок (лимит Better Auth — 3 входа за 10 секунд с IP): локально 60
 * одновременных неверных паролей с 20 IP при 29 неудачах дали 32 проверки
 * пароля вместо одной. С одного IP это не больше двух лишних попыток. Строгий
 * порог требует засчитывать попытку до проверки пароля и откатывать после
 * успеха — не сделано.
 *
