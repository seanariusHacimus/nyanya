 ...
 *
 * Для публикации не нужен ни один документ (решение владельца, 2026-09-13):
 * анкету без фотографии показывают с аватаром по полу и без значка. Документы
 * специалист предоставляет по своему желанию, и значок стандартного профиля
 * ничего о них не утверждает.
 *
...
export type DocumentSummary = {
  ...
  allApproved: boolean;
  /**
   * Фотография принята. До 2026-09-13 это был минимум для публикации;
   * теперь фото необязательно (решение владельца): анкету без него можно
   * опубликовать, семья увидит аватар по полу. Принятое фото по-прежнему
   * даёт «Стандартный профиль» вместо «Неполного».
   */
  photoApproved: boolean;
  /**
   * Фотография загружена, но модератор её ещё не смотрел. Единственное, что
   * блокирует публикацию: показать семье непроверенный снимок нельзя.
   */
...
export function summarizeDocuments(
  ...
  return {
    ...
    photoPending: byType.get("profile_photo") === "pending",
  };
}
...
export function deriveVerificationLevel(
   ...
   *
   * Без принятой фотографии уровень остаётся «Неполный профиль»: с
   * 2026-09-13 такую анкету администратор всё же может опубликовать (семья
   * увидит аватар по полу), но значок честно скажет, что фото нет.
   *
...
export const VERIFICATION_MEANING: Record<VerificationLevel, string> = {
  unverified:
    "Принятой фотографии нет: в каталоге показывается аватар по полу, значка у анкеты не будет. Публиковать можно.",
